const employees = [
    ...
    { id: 3, name: 'Bob Johnson', age: 35, department: 'Finance', salary: 60000, specialisation:"Rust"},
...
function findEmployeeById(employeeId) {
    ...
    function specialisation(lang) {
        const foundEmployee = employees.find(employee => employee.specialisation === lang);
        if (foundEmployee) {
        document.getElementById('employeesDetails').innerHTML =`<p>${foundEmployee.id}: ${foundEmployee.name}: ${foundEmployee.name} - ${foundEmployee.department} - $${foundEmployee.salary}</p>`;
        }
        else{
